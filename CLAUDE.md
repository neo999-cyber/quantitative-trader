...
00000. `docs/27_STAGE1_SCOPE.md` — **from 18 September 2026**: the ledger is built (`docs/26` status); the Codex build pack was assessed and only its stage-1 execution scope (B10–B16 for the maker-fill study, `docs/24`) is adopted, nothing before 1 October; C6 unlock fade is registered (`docs/prereg/p2_unlock_fade_v1.md`) and S1 index deletions is built as the backup (`docs/prereg/p2_index_deletion_v1.md`, not registered).
0000. `docs/26_SESSION_BRIEF_LEDGER.md` — **the live task from 18 September 2026**: the position/cash ledger (review 22's first ticket), state on handover, what runs unattended, the rules. Then `docs/25` and `docs/23`.
000. `docs/23_PROGRAMME_2_REPORT.md` — **Programme 2's report (17 September 2026).** Eight pre-registered candidates gated, none passed; per-family table, what it would take, what keeps running. The counter is at eight; nothing is registered without a new input.
