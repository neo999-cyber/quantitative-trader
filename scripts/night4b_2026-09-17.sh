...
stage() { echo "$(date -u +%FT%TZ) [night4b] $1" >> $STATUS; }
stage "start (both-leg metrics complete; released from the wait on the USDC-pair workers)"
stage "metrics pull finished: $(ls $QR_ROOT/mirror/binance/data/futures/um/daily/metrics | wc -l) symbols"
$QR data funding-ingest --metrics --symbols $(cat $LOG/both_syms.txt) > $LOG/night4_metrics_ingest.log 2>&1
stage "metrics ingest exit=$?"
$PY scripts/c5_universe_check.py > $LOG/night4_c5_check.log 2>&1
rc=$?
stage "c5 universe check exit=$rc"
if [ $rc -ne 0 ]; then stage "C5 NOT registered (check failed); stopping"; exit 0; fi
