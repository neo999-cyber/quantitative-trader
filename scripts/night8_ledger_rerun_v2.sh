#!/bin/zsh
# Review 22, step 8, second pass: the frozen grids on the ledger after the cash-interest and NaN-turnover fixes (docs/20, 18 Sep).
# Reports go to reports/ledger_engine_v2/; the first pass (interest-contaminated) stays in reports/ledger_engine/.
#
...
REPORTS=$QR_ROOT/reports; BEFORE=$REPORTS/weights_engine; AFTER=$REPORTS/ledger_engine_v2
mkdir -p $BEFORE $AFTER
stage() { echo "$(date -u +%FT%TZ) [night8] $1" >> $STATUS; }
...
rerun() {
  ...
  $QR gates --hypothesis $hyp --engine ledger "$@" > $LOG/night8_$hyp.log 2>&1
  stage "$hyp exit=$?"
  for f in $REPORTS/$hyp.md $REPORTS/$hyp.json $REPORTS/${hyp}_variant_returns.parquet; do
    [[ -f $f ]] && mv $f $AFTER/
  done
...
stage "start (clean ledger re-run after the interest and turnover fixes; owner said yes on 18 September 2026 afternoon)"
...
$QR trial verify > $LOG/night8_verify.log 2>&1
stage "verify $(head -1 $LOG/night8_verify.log)"
$PY scripts/ledger_before_after.py $BEFORE $AFTER > $LOG/night8_before_after.md 2>&1
stage "table -> $LOG/night8_before_after.md"
stage "done"
